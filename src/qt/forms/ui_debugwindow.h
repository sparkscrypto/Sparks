...
public:
    ...
    void retranslateUi(QWidget *RPCConsole)
    {
        ...
        openDebugLogfileButton->setToolTip(QApplication::translate("RPCConsole", "Open the Sparks Core debug log file from the current data directory. This can take a few seconds for large log files.", 0));
#endif // QT_NO_TOOLTIP
        ...
        label_repair_salvage->setText(QApplication::translate("RPCConsole", "-salvagewallet: Attempt to recover private keys from a corrupt wallet.dat.", 0));
        label_repair_rescan->setText(QApplication::translate("RPCConsole", "-rescan: Rescan the block chain for missing wallet transactions.", 0));
        label_repair_zap1->setText(QApplication::translate("RPCConsole", "-zapwallettxes=1: Recover transactions from blockchain (keep meta-data, e.g. account owner).", 0));
        label_repair_zap2->setText(QApplication::translate("RPCConsole", "-zapwallettxes=2: Recover transactions from blockchain (drop meta-data).", 0));
        label_repair_upgrade->setText(QApplication::translate("RPCConsole", "-upgradewallet: Upgrade wallet to latest format on startup. (Note: this is NOT an update of the wallet itself!)", 0));
        ...
        label_repair_reindex->setText(QApplication::translate("RPCConsole", "-reindex: Rebuild block chain index from current blk000??.dat files.", 0));
        wallet_path->setText(QApplication::translate("RPCConsole", "Wallet Path", 0));
