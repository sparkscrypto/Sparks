...
public:
    ...
    void setupUi(QDialog *AskPassphraseDialog)
    {
        ...
        mixingOnlyCheckBox = new QCheckBox(AskPassphraseDialog);
        mixingOnlyCheckBox->setObjectName(QStringLiteral("mixingOnlyCheckBox"));
        mixingOnlyCheckBox->setEnabled(true);
        mixingOnlyCheckBox->setVisible(false);

        formLayout->setWidget(4, QFormLayout::LabelRole, mixingOnlyCheckBox);
    ...
    void retranslateUi(QDialog *AskPassphraseDialog)
    {
        ...
        mixingOnlyCheckBox->setToolTip(QApplication::translate("AskPassphraseDialog", "Serves to disable the trivial sendmoney when OS account compromised. Provides no real security.", 0));
#endif // QT_NO_TOOLTIP
        mixingOnlyCheckBox->setText(QApplication::translate("AskPassphraseDialog", "Only for mixing via PrivateSend", 0));
    } // retranslateUi
